...
local function createBind(option, parent)
    ...
    inputService.InputEnded:connect(function(input)
        if input.KeyCode.Name == option.key or input.UserInputType.Name == option.key or (input.UserInputType.Name == "MouseMovement" or input.UserInputType.Name == "Touch") then
            if loop then
                loop:Disconnect()
                loop = nil
                option.callback(true)
...
inputService.InputChanged:connect(function(input)
    if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and library.cursor then
        ...
        library.cursor.Position = (UDim2.new(0, mouse.X - 2, 0, mouse.Y - 2) or UDim2.new(0, input.Position.X - 2, 0, input.Position.Y - 2))
    end
